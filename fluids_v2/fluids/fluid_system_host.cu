...
CUDA_SAFE_CALL(cudaMalloc ( (void**) &(vertices_number), dim*sizeof(int)));
...
void SPH_ComputeForceCUDA ()
{
 ...
 /*  for(int i=0;i<500;i++)
   {
     cudaMemcpy(temp,out_matrix_[i], sizeof(float)*9, cudaMemcpyDeviceToHost);
...
void SPH_ExclusiveScan(int * *vnumber, int & number)
{
	...
	//ExclusiveScan(vertices_number, fcuda.xDim*fcuda.yDim*fcuda.zDim);
	(*vnumber)=vertices_number;
	number=fcuda.xDim*fcuda.yDim*fcuda.zDim;
