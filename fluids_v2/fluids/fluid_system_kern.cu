...
__global__ void computeDensity(float * out_density, float *in_G[], float3 * in_mean_p,char * bufPnts ,int numPnt,int x, int y,int z,float size_voxel)//
{
	...
	if(ndx<x&&ndy<y&&ndz<z) 
	{
		...
		for(int i=0;i<numPnt;i++)
		{
			...
			float3 dis=(posi-in_mean_p[i])*simData.sim_scale ;
			...
			if(l<simData.smooth_rad)
			{
				...
				den+=G*m*m*m*m/(*(float*) (data + OFFSET_DENS));
			}

		}
		den*=simData.pmass*simData.poly6kern;
...
__global__ void compute_vertices_number(int * case_number,int * vertices_number, float * color_density, int * d_numVertsTable,  int x, int y, int z, float isolevel)
{
	...
	if(ndx<x&&ndy<y&&ndz<z)
	{
		...
		color[7]=color_density[index+x*y];
...
__device__ void computeVetexNormal(int edge, float * color_density, uint ** d_edgeVertices, float3 * v,float3 * normals, float3 * normal, float3 * position, float size_voxel, float  isolevel)
{
   ...
   if(fabs(density2-isolevel)<EPS)
         {
         *normal=normal2;
         *position=v2;
         return;
         }
       if(abs(density1-isolevel)<EPS)
         {
         *normal=normal1;
         *position=v1;
         return;
        ...
        float weight=(isolevel-density2)/(density1-density2);
        
        *normal=lerp(normal1,normal2,weight);
        *position=lerp(v1,v2,weight);
...
__global__ void marching_cube(float3 * points, float3 * normals, uint * d_numVertsTable, uint ** d_edgeVertices, uint ** d_triTable, float * color_density,int x, int y, int z, float size_voxel, float isolevel, uint * vertices_number, uint * case_number )
{
	...
	if(ndx<x&&ndy<y&&ndz<z)
	{
		...
		uint cases=case_number[index];
		uint numberVer=d_numVertsTable[cases];
		...
		while(numberVer!=0)
		{
			...
			if(d_triTable[cases][i]!=-1)//////
			{
				...
				computeVetexNormal(d_triTable[cases][i], color, d_edgeVertices, v, normal, &normals[ver_index++], &points[ver_index++], size_voxel, isolevel);
				;
				numberVer--;
			}
			i++;
...
{		
	...
	*/
	...
	if ( ndx < numPnt ) {
		...
		speedLimit2 = speedLimit*speedLimit;
		particleRadius = 0.004;//simData.particleRadius;
