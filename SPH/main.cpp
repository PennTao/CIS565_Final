...
void drawScene ( float* viewmat, bool bShade )
{
	if ( iShade <= 1 && bShade ) {		
		...
		glEnd ();
		//psys.SPH_DrawTriangles(&viewmat[0]);
	...
	psys.Draw ( &viewmat[0], 0.8 );				// Draw particles		
		...
		psys.Draw ( &viewmat[0], 0.55 );	
		//psys.SPH_DrawTriangles(&viewmat[0]);
  psys.SPH_DrawTriangles(&viewmat[0]);		// Draw particles
	}
}
...
void keyboard_func ( unsigned char key, int x, int y )
{
	...
	case 'M': case 'm': {
		...
		psys.SPH_Test ( psys_demo, psys_nmax );
		} break;
	case 'N': case 'n': {
		psys_nmax /= 2;
	...
	case '\'': case ';':	psys.SPH_Test ( psys_demo, psys_nmax ); break;
	case 'r': case 'R':		psys.SPH_Test ( psys_demo, psys_nmax ); break;  
	case '[':
		psys_demo--;
