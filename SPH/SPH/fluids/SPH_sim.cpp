...
{
		this->triangle_number=max_triangle_number;
		this->points_List=new float3[triangle_number];
...
void FluidSystem::Run ()
{
	...
	//allocateCUDAspace(&out_matrix, &out_mean_p);
		...
		this->triangle_number=SPH_ExclusiveScan();

	


		if(this->triangle_number>max_triangle_number)
		{
			delete points_List;
