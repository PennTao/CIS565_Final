...
//		int id = omp_get_num_threads();
		...
		//	acc *= (1/m_Param[SPH_PMASS]);
			...
			pnt->vel_eval += vnext;
			pnt->vel_eval *= 0.5;					// v(t) = [v(t-1/2) + v(t+1/2)] * 0.5		
			pnt->vel = vnext;
			vnext *= m_DT/simScale;
...
void FluidSystem::SPH_Test ( int n, int nmax )
{
	...
	case 0:		
		...
		m_Vec [ SPH_VOLMAX ].Set ( 25, 20, 30 );		
		
		m_Vec [ SPH_INITMIN ].Set ( -20, -20, -10 );
		m_Vec [ SPH_INITMAX ].Set ( 20, 20, 10 );

		m_Param [ FORCE_XMIN_SIN ] =6.0;
		m_Param [ BOUND_ZMIN_SLOPE ] = 0.03;
		m_Vec [ PLANE_GRAV_DIR ].Set ( 0.0, 0, -9.8 );
		break;

	case 1:		//gravity inverse
		m_Vec [ SPH_VOLMIN ].Set ( -25, -20, 0 );
		m_Vec [ SPH_VOLMAX ].Set ( 25, 20, 30 );			
		m_Vec [ SPH_INITMIN ].Set ( -20, -20, -10 );
		m_Vec [ SPH_INITMAX ].Set ( 20, 20, 10 );
	...
	Grid_InsertParticles ();									// Insert particles
		...
		FluidSetupCUDA ( NumPoints(), sizeof(Fluid), *(float3*)&  m_Vec[SPH_VOLMIN], *(float3*)& m_Vec[SPH_VOLMAX], *(float3*)& m_GridRes, *(float3*)& m_GridSize, (int) m_Vec[EMIT_RATE].x );
		//FluidSetupCUDA ( NumPoints(), sizeof(Fluid),make_float3(),  *(float3*)m_Vec [ SPH_VOLMAX ].Data, *(float3*)& m_GridRes, *(float3*)& m_GridSize, (int) m_Vec[EMIT_RATE].x );
